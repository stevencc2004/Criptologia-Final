...
def main():
  ...
  EduChain debe detectar la alteración en 3 niveles independientes:
    ...
    print(f"    ¿El nuevo hash empieza con '000'?         {'SÍ' if hash_con_cambio.startswith('000') else 'NO ← PoW ROTO ✓'}" if False else f"    ¿El nuevo hash empieza con '000'?         {'SÍ' if hash_con_merkle_nuevo.startswith('000') else 'NO ← PoW ROTO ✓'}")
    print(f"\n    Conclusión: El efecto avalancha del SHA-256 hace que un solo cambio de")
    print(f"    bit en la entrada produzca un hash completamente diferente. El atacante")
    print(f"    necesitaría REMINAR el bloque (y todos los posteriores) para ocultar la alteración.")
    esperar(0.3)
